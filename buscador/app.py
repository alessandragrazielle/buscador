...
def main(diretorio='./pages', caminho_json_scores='./pontuacoes.json'):
    ...
    # key=lambda x: (x[6], x[2], x[5], x[1]): Aqui, uma função lambda é definida para fornecer a chave de classificação. Esta função lambda pega cada elemento x da lista resultados e retorna uma tupla (x[6], x[2], x[5], x[1]). Esta tupla define a ordem de classificação: primeiro por x[6], depois por x[2], depois por x[5] e finalmente por x[1]
    # o parâmetro key permite que você especifique uma função que determine como os elementos da lista devem ser comparados durante a ordenação. A função key é aplicada a cada elemento da lista e retorna um valor que será usado para classificar os elementos.
    #o reverse=True significa que a ordenacao vai ser em ordem decrescente
